...
export default function LoginPage() {
  ...
  // Místo localStorage teď Clerk sám ví, jestli je uživatel přihlášený.
  // Pokud chceš po přihlášení automaticky přesměrovat z této stránky pryč:
  const handleGoogleLogin = async () => {
      if (!isLoaded || !signIn) return;

      try {
        // Spustí Google OAuth proces přizpůsobený pro HashRouter
        await signIn.authenticateWithRedirect({
          strategy: "oauth_google",
          redirectUrl: "/#/sso-callback", 
          redirectUrlComplete: "/#/panel", 
        });
      } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <div className="relative w-full max-w-md px-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
          ...
          {/* Google Login Button - Teď napojený na Clerk */}
          <button
            onClick={handleGoogleLogin}
            disabled={!isLoaded}
            className="w-full flex items-center justify-center gap-3 bg-white dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-xl px-6 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-600 transition-all duration-200 hover:shadow-md group disabled:opacity-50"
          >
